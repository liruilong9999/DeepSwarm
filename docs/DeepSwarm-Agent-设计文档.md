...
为避免多个智能体同时改主工作目录，逻辑层仍采用“主工作区 + 智能体补丁层”，但真正的合并引擎改为 Git(版本控制) 三路合并：

1. `Worker` 基于某个 `base_revision` 读取代码并在临时工作目录产出补丁
2. 运行时记录补丁内容、受影响文件集合和 `base_revision`
3. `Reviewer` 或 `Coordinator` 审查补丁后，`PatchMerger` 在独立 merge workspace(合并工作区) 中执行 `git apply --3way`
4. 三路合并成功时，写回主工作区并推进 `base_revision`
5. 三路合并失败时，先在最新 head(头提交) 上自动重试 1 次
6. 若仍失败，则把相关任务标记为 `merge_blocked`，并触发以下兜底链：
   - 同文件冲突任务转为串行执行
   - 必要时把冲突任务退回 `Coordinator` 重新拆分
   - 连续失败则升级为人工介入或回滚本次 Turn(轮次) 未合并部分

额外约束：

- 出现文件重命名、跨文件大重构、依赖升级时，默认进入 exclusive scope(排他范围)，同一时刻只允许 1 个 `Worker` 修改该范围
- 首版不采用“每个 Worker 一个常驻完整 worktree(工作树)”作为默认方案，但允许在高冲突模式下临时启用独立 merge workspace
...
3. 参数不合法时，不执行工具，而是把“非法参数 + 历史上下文”回灌给模型，要求重新生成
...
1. SQLite：结构化状态，且必须开启 WAL(预写日志) 模式
...
- 未完成的模型调用不做网络层续传，而是以最近持久化消息历史重新发起
...
流式输出建议后台统一产出事件流。首版 CLI 可以直接消费内部事件流；桌面端后续接入时，再在 WebSocket(双向实时通道) 与 Server-Sent Events(服务器推送事件) 之间做最终取舍。
...
| 指标 | 目标 |
|---|---|
| 前台 CLI 会话首 token 延迟 | p95 小于 2 秒，不含真实工具执行耗时 |
| 取消响应时间 | p95 小于 2 秒 |
...
| 多智能体写冲突 | 同会话多个 worker 改同一文件 | 用补丁层 + Git 三路合并 + 冲突串行化 |
...
| DeepSeek 模型变更或弃用 | 模型名、上下文、价格调整 | 启动时读取 `/models` 校验，拒绝使用已弃用模型别名 |
